...
const DEFAULT_TIERS = [
  { id: 'weak',   label: 'تأییدهای معمولی', pct: 0.25 },
  { id: 'strong', label: 'تأییدهای قوی (هم‌جهتی BTC، حجم خوب، RSI اشباع)', pct: 0.5 }
];
...
const FIELD_ALIASES = {
  ...
  riskPct: ['درصد ریسک', 'risk%', 'risk percent', 'risk', 'risk pct', 'risk (%)'],
...
const DEFAULT_GNOTES = {
  ...
  dailyProfitTarget: '', weeklyProfitTarget: '',
  freeNotes: ''
};
...
function loadTrades(){
  try{
    ...
    // مهاجرت فرمت قدیمی win/loss/breakeven -> tp/sl/riskfree
    const map = { win: 'tp', loss: 'sl', breakeven: 'riskfree' };
    return data.map(t => ({ ...t, outcome: map[t.outcome] || t.outcome || 'open' }));
  }catch(e){ return []; }
}
...
function render(){
  ...
  if(STATE.view === 'notes') c.innerHTML = viewGlobalNotes();
...
function viewNewTrade(){
  ...
  const tierOptions = STATE.tiers.map(t=>`<option value="${t.id}">${t.label} — ریسک ${fmtNum(t.pct,2)}٪</option>`).join('');
    ...
    <form id="form-new-trade">
      ...
      <div class="card">
        ...
        <div class="grid cols-2">
          <div class="checkline"><input type="checkbox" name="trendAlign" id="trendAlign"><label for="trendAlign">هم‌جهت با روند 4 ساعته</label></div>
          <div class="checkline"><input type="checkbox" name="btcConfirm" id="btcConfirm"><label for="btcConfirm">تأیید BTC</label></div>
          <div class="checkline"><input type="checkbox" name="btcPairConfirm" id="btcPairConfirm"><label for="btcPairConfirm">تأیید هم‌جهتی با جفت BTC</label></div>
          <div class="checkline"><input type="checkbox" name="hwcConfirm" id="hwcConfirm"><label for="hwcConfirm">تأیید HWC</label></div>
          <div class="checkline"><input type="checkbox" name="mwcConfirm" id="mwcConfirm"><label for="mwcConfirm">تأیید MWC</label></div>
          <div class="checkline"><input type="checkbox" name="lwcConfirm" id="lwcConfirm"><label for="lwcConfirm">تأیید LWC</label></div>
          <div class="checkline"><input type="checkbox" name="level4hBreak" id="level4hBreak"><label for="level4hBreak">شکست سطح 4H/1D</label></div>
          <div class="checkline"><input type="checkbox" name="volConfirm" id="volConfirm"><label for="volConfirm">تأیید حجم</label></div>
          <div class="checkline"><input type="checkbox" name="candleShapeConfirm" id="candleShapeConfirm"><label for="candleShapeConfirm">تأیید شکل کندل</label></div>
          <div class="checkline"><input type="checkbox" name="rsiConfirm" id="rsiConfirm"><label for="rsiConfirm">تأیید RSI</label></div>
          <div class="checkline"><input type="checkbox" name="candleClosed" id="candleClosed"><label for="candleClosed">بعد از بسته‌شدن کامل کندل وارد شدم</label></div>
          <div class="checkline"><input type="checkbox" name="notWeekendRange" id="notWeekendRange"><label for="notWeekendRange">بازار رنج یا نزدیک بسته‌شدن ماهانه نیست</label></div>
        </div>
      ...
      <div class="card">
        ...
        <div class="grid cols-3">
          ...
          <div class="field"><label>درصد ریسک حساب</label><input type="number" step="any" name="riskPct" placeholder="خودکار از سطح تأیید"></div>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="grid cols-3">
          ...
          <div class="field"><label>R محقق‌شده</label><input type="number" step="any" name="realizedR" placeholder="مثلاً 2.5 یا -1"></div>
          <div class="field"><label>درصد بسته‌شده تا الان</label><input type="number" step="any" name="closedPct" placeholder="0 تا 100"></div>
        </div>
        <div class="field"><label>یادداشت / درس گرفته‌شده</label><textarea name="notes" placeholder="چی خوب پیش رفت، چی نه؟"></textarea></div>
      </div>
...
function handleNewTradeSubmit(e){
  ...
  const fields = {
    ...
    trendAlign: !!f.get('trendAlign'),
    btcConfirm: !!f.get('btcConfirm'),
    btcPairConfirm: !!f.get('btcPairConfirm'),
    hwcConfirm: !!f.get('hwcConfirm'),
    mwcConfirm: !!f.get('mwcConfirm'),
    lwcConfirm: !!f.get('lwcConfirm'),
    level4hBreak: !!f.get('level4hBreak'),
    volConfirm: !!f.get('volConfirm'),
    candleShapeConfirm: !!f.get('candleShapeConfirm'),
    rsiConfirm: !!f.get('rsiConfirm'),
    candleClosed: !!f.get('candleClosed'),
    notWeekendRange: !!f.get('notWeekendRange'),
    justification,
    entry: parseFloat(f.get('entry')),
    stop: parseFloat(f.get('stop')),
    riskPct: f.get('riskPct') ? parseFloat(f.get('riskPct')) : (tier ? tier.pct : null),
    ...
    flagEarlyEntry: !f.get('candleClosed')
  };
    ...
    <div class="card" style="padding:0; overflow-x:auto;">
      <table>
        <thead><tr>
          ...
          <th>ریسک٪</th><th>R</th><th>وضعیت</th><th>هشدارها</th><th></th>
        </tr></thead>
        <tbody>${rows}</tbody>
      </table>
    </div>`}
...
function tradeRow(t){
  ...
  return `
    <tr>
      ...
      <td class="num">${fmtNum(t.riskPct,2)}٪</td>
...
function viewRules(){
  ...
  const tierRows = STATE.tiers.map((t, i)=>`
    <div class="tier-row">
      ...
      <input type="number" step="any" data-tier-pct="${t.id}" value="${t.pct}">
...
/* ============================== یادداشت‌ها (کلی) ============================== */
function viewGlobalNotes(){
  const n = STATE.globalNotes;
  return `
    <h2 class="section-title">یادداشت‌ها</h2>
    <p class="section-sub">حد ضرر و حد سود روزانه/هفتگی و هر یادداشت کلی دیگه‌ای که می‌خوای همیشه جلوی چشمت باشه — مستقل از معاملات، همیشه اینجا در دسترسه.</p>

    <div class="card">
      <h3>محدودیت‌های ریسک</h3>
      <div class="grid cols-2">
        <div class="field">
          <label>حد ضرر روزانه</label>
          <input type="text" data-gnote="dailyLossLimit" value="${escapeHtml(n.dailyLossLimit)}" placeholder="مثلاً 2% یا 2R">
        </div>
        <div class="field">
          <label>حد ضرر هفتگی</label>
          <input type="text" data-gnote="weeklyLossLimit" value="${escapeHtml(n.weeklyLossLimit)}" placeholder="مثلاً 5% یا 5R">
        </div>
        <div class="field">
          <label>حد سود روزانه</label>
          <input type="text" data-gnote="dailyProfitTarget" value="${escapeHtml(n.dailyProfitTarget)}" placeholder="مثلاً 3% یا 3R">
        </div>
        <div class="field">
          <label>حد سود هفتگی</label>
          <input type="text" data-gnote="weeklyProfitTarget" value="${escapeHtml(n.weeklyProfitTarget)}" placeholder="مثلاً 8% یا 8R">
        </div>
      </div>
      <div class="hint">به هر واحدی که خودت دنبال می‌کنی بنویس — درصد، R، یا حتی مبلغ دلاری.</div>
    </div>

    <div class="card">
      <h3>یادداشت‌های عمومی</h3>
      <textarea data-gnote="freeNotes" style="min-height:200px;" placeholder="هر نکته یا یادآوری کلی دیگه‌ای که می‌خوای همیشه در دسترس باشه...">${escapeHtml(n.freeNotes)}</textarea>
    </div>
  `;
}
...
function attachViewHandlers(){
  ...
  if(STATE.view === 'rules'){
    ...
    document.querySelectorAll('[data-tier-pct]').forEach(inp=>{
      inp.addEventListener('change', ()=>{
        const t = findTier(inp.dataset.tierPct);
        if(t){ t.pct = parseFloat(inp.value) || 0; saveTiers(); }
      });
    });
    ...
    addTierBtn && addTierBtn.addEventListener('click', ()=>{
      STATE.tiers.push({ id: slugify('tier'), label: 'سطح جدید', pct: 0.25 });
      saveTiers();
      render();
    });
  }
...
document.getElementById('btn-export-xlsx').addEventListener('click', ()=>{
  ...
  const rows = STATE.trades.map(t=>({
    ...
    'هم‌جهت روند 4H': t.trendAlign ? 'بله' : 'خیر',
    'تأیید BTC': t.btcConfirm ? 'بله' : 'خیر',
    'تأیید هم‌جهتی جفت BTC': t.btcPairConfirm ? 'بله' : 'خیر',
    'تأیید HWC': t.hwcConfirm ? 'بله' : 'خیر',
    'تأیید MWC': t.mwcConfirm ? 'بله' : 'خیر',
    'تأیید LWC': t.lwcConfirm ? 'بله' : 'خیر',
    'شکست سطح 4H/1D': t.level4hBreak ? 'بله' : 'خیر',
    'تأیید حجم': t.volConfirm ? 'بله' : 'خیر',
    'تأیید شکل کندل': t.candleShapeConfirm ? 'بله' : 'خیر',
    'تأیید RSI': t.rsiConfirm ? 'بله' : 'خیر',
    'بعد از بسته‌شدن کندل': t.candleClosed ? 'بله' : 'خیر',
...
// بعضی فایل‌ها ریسک رو به‌صورت کسر می‌نویسن (0.0025 یعنی 0.25٪) و بعضی مستقیم درصد (0.5 یعنی 0.5٪).
// اگه عدد خیلی کوچیکه (کمتر از 0.02) فرض می‌کنیم کسره و در 100 ضرب می‌شه.
function parseRiskPct(v){
  const n = parseNum(v);
  if(n === null) return null;
  return Math.abs(n) < 0.02 ? n * 100 : n;
}
    ...
    try{
      ...
      rows.forEach(row=>{
        ...
        STATE.trades.push({
          ...
          trendAlign: false,
          btcConfirm: parseYesNo(m.btcConfirm),
          btcPairConfirm: parseYesNo(m.btcPairConfirm),
          hwcConfirm: parseYesNo(m.hwcConfirm),
          mwcConfirm: parseYesNo(m.mwcConfirm),
          lwcConfirm: parseYesNo(m.lwcConfirm),
          level4hBreak: parseYesNo(m.level4hBreak),
          volConfirm: parseYesNo(m.volConfirm),
          candleShapeConfirm: parseYesNo(m.candleShapeConfirm),
          rsiConfirm: parseYesNo(m.rsiConfirm),
          candleClosed: true,
          notWeekendRange: true,
          justification,
          entry: parseNum(m.entry),
          stop: parseNum(m.stop),
          riskPct: parseRiskPct(m.riskPct),
